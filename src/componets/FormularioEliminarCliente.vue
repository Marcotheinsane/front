...
// Emitir evento
const emit = defineEmits(['cliente-eliminado'])
...
const eliminarCliente = async () => {
  ...
  try {
    ...
    await personasAPI.delete(props.cliente.id)
    
    console.log('Cliente eliminado exitosamente')
    
    // Emitir evento para recargar listado
    emit('cliente-eliminado')
    cerrarConfirmacion()

  } catch (err) {
    error.value = err.response?.data?.mensaje || err.message || 'Error al eliminar cliente'
    ...
    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full z-50 overflow-hidden">
      ...
      <div class="bg-gradient-to-r from-red-600 to-red-700 text-white p-6">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="text-xl font-bold">¿Eliminar cliente?</h2>
      ...
      <div class="p-6">
        ...
        <div class="mb-6 p-4 bg-red-50 border-l-4 border-red-300 rounded-lg">
          ...
          <p class="font-bold text-red-900 text-lg">{{ props.cliente.nombre }} {{ props.cliente.apellido }}</p>
          <p class="text-sm text-gray-600 mt-1">RUT: {{ props.cliente.rut }}</p>
        </div>
        ...
        <div class="mb-6 p-4 bg-yellow-50 border-l-4 border-yellow-300 rounded-lg">
          <p class="text-yellow-800 text-sm">
            <strong>Advertencia:</strong> Todos los registros de asistencia de este cliente también serán eliminados.
          </p>
        </div>
